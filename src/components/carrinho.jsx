      ...
      {itens.map(item => (
        <div key={item.id} className="flex justify-between items-center border-b border-gray-700 py-2">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => diminuir(item.id)} className="bg-amber-500 text-white w-6 h-6 rounded">-</button>
            <span className="text-white text-sm">{item.quantidade}</span>
            <button onClick={() => aumentar(item.id)} className="bg-amber-500 text-white w-6 h-6 rounded">+</button>
          </div>
